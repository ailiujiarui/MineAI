...
export function initBot(username) {
    const options = {
        ...
        auth: settings.auth,
        version: mc_version,
        checkTimeoutInterval: 60000,  // 60s keep-alive check (default 30s) 鈥?reduces disconnects on slow servers
    }
    ...
    // Compatibility shim for older plugins still subscribing to deprecated physicTick.
    ...
    bot.on = function(event, listener) {
        return originalOn(event === 'physicTick' ? 'physicsTick' : event, listener);
    };
    bot.once = function(event, listener) {
        return originalOnce(event === 'physicTick' ? 'physicsTick' : event, listener);
    };
    bot.removeListener = function(event, listener) {
        return originalRemoveListener(event === 'physicTick' ? 'physicsTick' : event, listener);
    };
