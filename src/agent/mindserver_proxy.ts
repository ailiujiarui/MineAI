...
class MindServerProxy {
    ...
    async connect(name, port) {
        ...
        this.socket.on('voice-transcript', async (data) => {
            try {
                console.log('[voice-debug][agent-proxy] received transcript:', JSON.stringify({
                    source: data?.source,
...
export function sendVoiceTranscriptToAgent(agentName, data) {
    serverProxy.getSocket().emit('voice-transcript', agentName, data);
}
