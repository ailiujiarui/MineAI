...
export const actionsList = [
    ...
    },
    {
        ...
        perform: async function (agent, profile_name) {
            if (!agent.voice_runtime) {
                return 'Voice runtime is not available.';
            }
            const changed = agent.voice_runtime.setVoiceProfile(profile_name);
            if (!changed) {
                return `Voice profile '${profile_name}' was not found.`;
            }
            return `Voice profile set to '${profile_name}'.`;
        }
    },
    {
        name: '!goToPlayer',
    ...
    {
        name: '!exploreForBlock',
        description: 'Search for a block, and if it is not nearby, move outward and retry until found or attempts run out.',
        params: {
            'type': { type: 'BlockName', description: 'The block type to search for while exploring.' },
            'search_range': { type: 'float', description: 'The local search radius used on each exploration attempt.', domain: [10, 512] },
            'move_distance': { type: 'float', description: 'How far to move between failed search attempts.', domain: [4, 128] }
        },
        perform: runAsAction(async (agent, block_type, range, move_distance) => {
            if (range < 32) {
                range = 32;
            }
            await skills.exploreForBlock(agent.bot, block_type, range, move_distance, 4);
        }, false, 12)
    },
    {
    ...
    {
        name: '!tillAndSow',
        description: 'Till farmland at the given coordinates and plant the given seed type.',
        params: {
            'x': { type: 'float', description: 'The x coordinate of the farmland block.', domain: [-Infinity, Infinity] },
            'y': { type: 'float', description: 'The y coordinate of the farmland block.', domain: [-64, 320] },
            'z': { type: 'float', description: 'The z coordinate of the farmland block.', domain: [-Infinity, Infinity] },
            'seed_type': { type: 'ItemName', description: 'Seed item to plant, such as wheat_seeds.' }
        },
