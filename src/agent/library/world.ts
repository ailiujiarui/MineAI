...
export function getNearbyDryEscapeCandidates(bot, distance=6) {
    ...
    for (const pos of airBlocks) {
        ...
        const waterNeighbors = neighbors.filter(block => block?.name === 'water').length;

        candidates.push({
            x: pos.x,
            y: pos.y,
            z: pos.z,
            solidGround: !!below && below.name !== 'air' && below.name !== 'water' && below.name !== 'lava',
            headClear: !head || head.name === 'air',
            waterNeighbors
        });
    }
...
export function isEntityWithinRange(bot, entity, maxDistance=16) {
    if (!entity?.position) {
        return false;
    }
...
function getHostileTargetScore(bot, entity) {
    const distance = bot.entity.position.distanceTo(entity.position);
    let score = -distance;
    if (entity.name === 'creeper') {
        score += distance <= 6 ? 20 : 10;
    }
    return score;
}

export async function selectHostileTarget(bot, maxDistance=16, options={}, deps={}) {
    const {
        currentTarget = null,
        currentTargetExpiresAt = 0
    } = options;
    const clearPath = deps.isClearPath || isClearPath;
    const now = deps.now || (() => Date.now());

    const isValidHostile = (entity) => mc.isHostile(entity) && isEntityWithinRange(bot, entity, maxDistance);

    if (currentTarget && currentTargetExpiresAt > now() && isValidHostile(currentTarget)) {
        if (await clearPath(bot, currentTarget)) {
            return currentTarget;
        }
    }

    const candidates = getNearbyEntities(bot, maxDistance)
        .filter(isValidHostile)
        .sort((left, right) => getHostileTargetScore(bot, right) - getHostileTargetScore(bot, left));

    for (const candidate of candidates) {
        if (await clearPath(bot, candidate)) {
            return candidate;
        }
    }
