...
import { chooseWaterEscapeTarget, shouldTriggerWaterEscape } from './library/waterRecovery.js';
...
const modes_list = [
    {
        ...
        update: async function (agent) {
            ...
            let blockBelow = bot.blockAt(bot.entity.position.offset(0, -1, 0));
            if (!block) block = {name: 'air'}; // hacky fix when blocks are not loaded
            if (!blockAbove) blockAbove = {name: 'air'};
            if (!blockBelow) blockBelow = {name: 'air'};

            const horizontalSpeed = Math.sqrt((bot.entity.velocity?.x || 0) ** 2 + (bot.entity.velocity?.z || 0) ** 2);
            if (block.name === 'water' || blockAbove.name === 'water') {
                ...
                const dryCandidates = world.getNearbyDryEscapeCandidates(bot, 6);
                const candidate = chooseWaterEscapeTarget(origin, dryCandidates);
                const nearestDryDistance = candidate
                    ? Math.sqrt((candidate.x - origin.x) ** 2 + (candidate.y - origin.y) ** 2 + (candidate.z - origin.z) ** 2)
                    : Infinity;
                const quickDryEscape = block.name === 'water' && blockAbove.name !== 'water' && nearestDryDistance <= 1.5;

                if (horizontalSpeed < 0.03) {
                    if (!this.water_stuck_since) {
                        this.water_stuck_since = Date.now();
                    }
                } else {
                    this.water_stuck_since = 0;
                }

                if (shouldTriggerWaterEscape({
                    blockAtFeet: block.name,
                    blockAtHead: blockAbove.name,
                    horizontalSpeed,
                    stuckSeconds: this.water_stuck_since ? (Date.now() - this.water_stuck_since) / 1000 : 0,
                    nearestDryDistance
                })) {
                    execute(this, agent, async () => {
                        if (candidate) {
                            await skills.goToPosition(bot, candidate.x, candidate.y, candidate.z, 0);
                            say(agent, 'Getting out of the water.');
                        } else {
                            await skills.goToSurface(bot);
                            say(agent, 'Trying to reach dry ground.');
                        }
                    });
                } else if (!quickDryEscape && !bot.pathfinder.goal) {
    ...
    },
    {
        ...
        active: false,
        update: async function (agent) {
            const enemy = await world.selectHostileTarget(agent.bot, 8);
            if (enemy) {
    ...
    },
    {
        ...
        active: false,
        update: async function (agent) {
            const enemy = await world.selectHostileTarget(agent.bot, 16);
            if (enemy) {
