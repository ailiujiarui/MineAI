...
const apiMap = await (async () => {
    ...
    const files = (await fs.readdir(__dirname))
        .filter(f => (f.endsWith('.ts') || f.endsWith('.js')) && !['_model_map.ts', '_model_map.js', 'prompter.ts', 'prompter.js'].includes(f));
...
export function createModel(profile) {
    ...
    const model = new apiMap[profile.api](profile.model, profile.url, profile.params, profile.wire_api);
    return model;
}
