...
export class Prompter {
    constructor(agent, profile) {
        ...
        const botRoot = resolveRunPath('bots', name);
        mkdirSync(botRoot, { recursive: true });
        writeFileSync(`${botRoot}/last_profile.json`, JSON.stringify(this.profile, null, 4), (err) => {
            if (err) {
    ...
    async _saveToFile(logFile, logEntry) {
        ...
        let logDir;
        if (task_id == null) {
            logDir = resolveRunPath('bots', this.agent.name, 'logs');
        } else {
            logDir = resolveRunPath('bots', this.agent.name, 'logs', task_id);
        }
